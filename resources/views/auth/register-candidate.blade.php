...
<div class="container mx-auto px-4 py-8">
    <div class="text-center mb-8">
        <h1 class="text-4xl md:text-5xl font-extrabold text-gray-900 mb-4">Devenir candidate</h1>
        <p class="text-lg text-gray-600">Rejoignez le concours Miss Élégance 2024</p>
    </div>

    <div class="bg-white rounded-xl shadow-lg p-6 md:p-8 max-w-2xl mx-auto">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">Formulaire d'inscription</h2>
        ...
        <form action="{{ route('candidates.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <div class="mt-1 flex items-center space-x-2">
                    <label class="cursor-pointer bg-pink-500 hover:bg-pink-600 text-white font-bold py-2 px-4 rounded-md shadow-sm transition duration-150 ease-in-out">
                        Choisir un fichier
                    ...
                    <span id="file-name" class="text-gray-500">Aucun fichier choisi</span>
                </div>
                <p class="text-xs text-gray-500 mt-1">Format accepté : JPG, PNG (max 5MB)</p>
                @error('main_photo')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="md:col-span-2">
                <label for="short_presentation" class="block text-sm font-medium text-gray-700">Présentation courte</label>
                <textarea id="short_presentation" name="short_presentation" rows="4" class="mt-1 block w-full border-gray-300 focus:border-pink-500 focus:ring-pink-500 rounded-md shadow-sm" placeholder="Parlez-nous de vous, vos passions, vos projets..." maxlength="500">{{ old('short_presentation') }}</textarea>
                <p class="text-xs text-gray-500 mt-1 text-right"><span id="char-count">0</span>/500 caractères</p>
            ...
            <div class="md:col-span-2 text-center text-xs text-gray-500 mt-4">
